...
class DiscordChatPage extends StatefulWidget {
  const DiscordChatPage({super.key});
...
class _DiscordChatPageState extends State<DiscordChatPage> {
  ...
  Future<void> check_server_connection() async {
    final is_connected = await api_service.check_server_status();
    setState(() {
      is_server_connected = is_connected;
    });
  }
  ...
  Future<void> send_message() async {
    ...
    final current_username = username?.trim() ?? '';

    if (text.isEmpty || current_username.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          ...
          if (!is_server_connected)
            Container(
              ...
              child: Row(
                ...
                children: [
                  const Icon(Icons.warning_amber, color: Color(0xFFFAA61A), size: 16),
                  const SizedBox(width: 8),
                  const Text(
                    'Connecting to Discord...',
                    style: TextStyle(color: Color(0xFFFAA61A), fontSize: 12),
                  ),
                ],
              ),
            ),

          if (replying_to != null) _buildReplyBanner(),
          ...
          Expanded(
            child: is_loading && messages.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) => _buildDiscordMessage(messages[index]),
                  ),
          ),
  ...
  Widget _buildDiscordMessage(Message message) {
    final is_me = message.username == username;
    final date_format = DateFormat('HH:mm');
          ...
          children: [
            ...
            Container(
              ...
              child: GestureDetector(
                onLongPress: () => shoe_options(message),
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: BorderRadius.only(
                      ...
                      bottomLeft: Radius.circular(is_me ? 18 : 4),
                      bottomRight: Radius.circular(is_me ? 4 : 18),
                    ),
                    boxShadow: [
                      BoxShadow(
                    ...
                    children: [
                        ...
                        Builder(
                          builder: (context) {
                            final originalMessage = message.reply_to_id != null
                                ? review_crate(message.reply_to_id !)
                                : null;
                      ...
                      Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text(
                              date_format.format(message.date_time),
                              style: TextStyle(
                                fontSize: 11,
                                color: is_me ? const Color(0xFF5865F2) : Colors.grey[400],
                              ),
                            ),
                            if (is_me) ...[
                              const SizedBox(width: 4),
  ...
  void shoe_options(Message message) {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              leading: const Icon(Icons.reply, color: Color(0xFF5865F2)),
              title: const Text(
                'Reply',
            ...
            ListTile(
              leading: const Icon(Icons.copy, color: Color(0xFF5865F2)),
              title: const Text(
                'Copy',
                style: TextStyle(color: Colors.white),
              ),
  ...
  Message? review_crate(String replyToId) {
    try {
